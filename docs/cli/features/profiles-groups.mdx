...
The `env_credentials` section maps keystore account names to environment variable names. Credentials are loaded from the system keystore (macOS Keychain / Linux Secret Service) before the sandbox is applied, then injected as environment variables.
...
The legacy field name `"secrets"` is accepted as an alias for backward compatibility.

See [Credential Injection](/cli/features/credential-injection) for details on storing credentials in the keystore.
