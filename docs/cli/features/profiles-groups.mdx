...
```
((default_profile_groups + profile.security.groups) - effective_group_exclusions) + profile.filesystem + CLI flags
```

1. **default_profile_groups** come from the built-in `default` profile
   The legacy `base_groups` field still backs that profile for compatibility, but is deprecated.
